...
⚠️ AHB 버스 사용 시 CPU 접근 제한

STM32에서 DMA는 AHB 버스를 통해 메모리에 직접 접근합니다.

따라서 DMA가 AHB를 점유하고 있는 동안, CPU는 같은 버스를 통해 메모리 접근이 일시적으로 제한됩니다.

이로 인해 CPU는 **메모리 대기 상태(stall)**에 들어갈 수 있으며, DMA는 Cycle Stealing 또는 Burst Mode로 전송 효율을 조절합니다.

타이밍 민감한 CPU 작업과 병행 설계 시 주의가 필요합니다.
### 🎞️ DMA를 활용한 고속 I/O 예시 (DVD Reader → RAM 전송)
- DMA는 DVD Reader로부터 데이터를 읽어 RAM에 저장할 수 있습니다.
- CPU는 소스 주소, 목적지 주소, 전송 크기를 설정만 하고 나면 DMA가 버스를 점유해 직접 데이터를 이동합니다.
- 전송 완료 후 인터럽트로 CPU에 통지되며, CPU는 다른 작업을 수행하거나 대기할 수 있습니다.
